...
summary_blsb %>%
  ...
  geom_text(data = summary_blsb_fert_bp, aes(label = round(w, 2), y = sd$y), vjust = 0.1, alpha = 0.75, color = "darkblue") +
...
summary_rust %>%
  ...
  geom_text(data = summary_rust_fert_bp, aes(label = treatment_cld,
                y = w + sd$ymax), vjust = -0.5, color="red", size = 5) +
  geom_text(data = summary_rust_fert_bp, aes(label = round(w, 2), y = sd$y), vjust = 0.1, alpha = 0.75, color = "darkblue") +
  labs(x = "Fertilizer Treatment Rate", 
       y = "Percent Incidence Common Rust",
